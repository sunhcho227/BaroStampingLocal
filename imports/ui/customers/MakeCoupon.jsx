...
export default () => {
  ...
  const closeModal = () => {
    ...
    navigate("/customers/mycoupons");
  };

  // 퍼블릭 쿠폰 이미지 경로 생성 함수
  const getPublicCouponImagePath = (storeUrlName) =>
    `/stores/${storeUrlName}_coupon.png`;
          ...
          <div className="makecoupon-stamp-info">
            ...
            <div className="coupon-usage-info">
              <span className="text_title_xs primary">
                스탬프 {store?.maxStamp}개
              </span>
              <span className="text_body_m">를 사용하여 </span>
              <span className="text_title_xs primary">
                {store?.couponInformation}
              </span>
              <span className="text_body_m">
                {" "}
                서비스 쿠폰을 발급받으실 수 있습니다.
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="makecoupon-selection">
          <div className="placeholder-box">
            <div className="placeholder-box">
              {loading ? (
                ...
                <img
                  src={getPublicCouponImagePath(store.storeUrlName)}
                  alt="Coupon"
                  className="coupon-image"
                  onError={(e) => (e.target.src = "/stores/default_coupon.png")}
                />
              ) : (
                <div></div>
              )}
          ...
          <div className="makecoupon-quantity-selector">
            ...
            <div className="product-quantity">
              <div
                onClick={decrementCouponCount}
                className="quantity-control-icon"
              >
                <img src="/icons/minus.svg" alt="minus" />
              </div>
              <div className="custom-inputbox-container inputbox_200px">
                <input
                  className="custom-inputbox no-spin text_align_center"
                  type="number"
                  value={couponCount}
          ...
          <button onClick={insertCoupon} className="btn-primary">
            쿠폰으로 교환하기
          </button>

          <div className="remaining-stamp-info">
            <div className="text_body_l">
              교환 후 남는 스탬프 : {calculateRemainingStamps()}개
            </div>
            <div className="text_body_xs">
              선택한 쿠폰 수에 따라 남는 스탬프가 계산됩니다.
            </div>
          </div>
        </div>

        <div className="makecoupon-footer">
          <div className="text_title_xs font_black_light">
            추가 정보 및 유의사항
          </div>
