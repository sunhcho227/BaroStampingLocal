...
import MyCouponsTap from "./MyCouponsTap";
...
export default () => {
  ...
  const { stamps, stores, coupons, isLoading } = useTracker(() => {
    const stamps = Stamps.find().fetch();
    const stores = Stores.find().fetch();
    const coupons = Coupons.find().fetch();

    const isLoading = !(stamps.length && stores.length && coupons.length);

    return { stamps, stores, coupons, isLoading };
  }, []);
        ...
        <div>
          ...
          <div className="mystore-store mystoreonly">
            ...
            {Stores.find({ _id: { $in: myAllStampStoresIds } }).fetch()
              .length === 0 ? (
              <p className="text_body_l">스탬프 적립중인 가게가 없습니다.</p>
            ) : (
          ...
          <div>
            <div className="mystore-store mystoreonly">
              ...
              {Stores.find({ _id: { $in: allStoreIds } }).fetch().length ===
              0 ? (
                <p className="text_body_l">방문한 가게가 없습니다.</p>
              ) : (
