...
const StoreImageSub = () => {
  ...
  // 퍼블릭 이미지 경로 생성 함수
  const getImagePath = (storeUrlName, type, index) => {
    if (!storeUrlName || !type) return `/stores/default_sub${index + 1}.png`;
    return `/stores/${storeUrlName}_${type}${index + 1}.png`;
  };
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
        {[...Array(3)].map((_, index) => (
          <div key={index} className="relative group">
            <img
              src={getImagePath(storeUrlName, "sub", index)}
              alt={`서브 이미지 ${index + 1}`}
              className="w-full h-48 object-cover rounded-lg"
              onError={(e) => {
                e.target.src = `/stores/default_sub${index + 1}.png`; // 기본 이미지로 대체
              }}
            />
          </div>
        ))}
