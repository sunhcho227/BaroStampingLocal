...
const StoreImageNormalCoupon = () => {
  ...
  const getImagePath = (storeUrlName, type) => {
    if (!storeUrlName || !type) return "/stores/default_normal_coupon.png";
    return `/stores/${storeUrlName}_coupon.png`;
  };
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
        <div className="relative group">
          <img
            src={getImagePath(storeUrlName, "normal-coupon")}
            alt="노멀쿠폰 이미지"
            className="w-full h-48 object-cover rounded-lg"
            onError={(e) => {
              e.target.src = "/stores/default_normal_coupon.png"; // 기본 이미지로 대체
            }}
          />
        </div>
      </div>
...
export default StoreImageNormalCoupon;
